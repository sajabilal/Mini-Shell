...

        {
            ...
            if (pid < 0) {//incase of failure of starting child process
                ...
                if (strcmp(command[0], "cd") == 0)//exception for "cd"
                    ...
                    sleep(1);
                    int pid1 = fork();//parent/child number 2  process initiation to get sched
                    if (pid1 < 0) {//incase of failure of starting child process
                        perror("fork() error");
                        return 1;
                    } else if (pid1 == 0) {//the second child process , contains the run of sched
                                                char string[50];
                        int ppid = getppid();
                        sprintf(string, "/proc/%d ", ppid);
                        printf("the path is: %s\n", string);
                        chdir(string);
                        const char *schedCommand[3];
                        schedCommand[0] = "cat";
                        schedCommand[1] = "/sched";
                        schedCommand[2] = NULL;
                        execvp(schedCommand[0], (char *const *) schedCommand);
                        printf("failed to execute sched\n");
                        exit(0);
                    } else {
                        sleep(1);
